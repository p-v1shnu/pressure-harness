...
async def run(extra_tools: int) -> None:
    ...
    async with stdio_client(params) as (reader, writer), ClientSession(reader, writer) as session:
        ...
        print(f"tools ({len(names)}): {', '.join(names[:8])}{' ...' if len(names) > 8 else ''}\n")

        expected = {
            "spike_read_file",
            "spike_write_file",
            "spike_overwrite_file",
            "spike_return_image",
            "spike_echo",
            "spike_whoami",
        }
        check(
            "all core tools advertised",
            expected <= set(names),
        ...
        if extra_tools:
            ...
            check(
                f"{extra_tools} padding tools registered",
                len(pads) == extra_tools,
                f"got {len(pads)}",
            )
